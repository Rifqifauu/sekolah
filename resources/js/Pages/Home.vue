<template>
  ...
  <section class="border-b-8 border-red relative overflow-hidden" data-aos="fade-up">
    <div class="relative h-60 md:h-[28em] w-full">
      ...
      <button @click="prevSlide"
        class="absolute top-1/2 left-4 transform -translate-y-1/2 z-40 text-white bg-red-200/30 hover:bg-red-200/60 p-2 rounded-full">
        ‹
      </button>
      <button @click="nextSlide"
        class="absolute top-1/2 right-4 transform -translate-y-1/2 z-40 text-white bg-red-200/30 hover:bg-red-200/60 p-2 rounded-full">
        ›
      </button>
    </div>

    <!-- Teks untuk Mobile -->
    <div class="block md:hidden bg-red-800 text-white p-4 text-left">
      ...
      <div class="max-w-4xl mx-auto">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 md:gap-8">
          ...
          <div data-aos="fade-right"
            class="text-center group bg-white rounded-2xl shadow-2xl p-4 md:p-6 border border-red-100">
            <div
              class="text-red-800 mb-3 text-3xl md:text-5xl transform group-hover:scale-110 transition-transform duration-300">
              <a href="/siswa"><i class="fas fa-user-graduate"></i></a>
            </div>
            <div class="text-xl md:text-3xl font-bold text-red-800 mb-1">{{ siswa }}</div>
            <div class="text-red-600 text-sm md:text-base font-medium">Siswa Aktif</div>
          </div>
          ...
          <div data-aos="fade-down"
            ...
            <div class="text-xl md:text-3xl font-bold text-red-800 mb-1">{{ guru }}</div>
            <div class="text-red-600 text-sm md:text-base font-medium">Guru</div>
          </div>
          ...
          <div data-aos="fade-down"
            ...
            <div class="text-xl md:text-3xl font-bold text-red-800 mb-1">{{ staf }}</div>
            <div class="text-red-600 text-sm md:text-base font-medium">Staf</div>
          </div>
          ...
          <div data-aos="fade-left"
            ...
            <div class="text-xl md:text-3xl font-bold text-red-800 mb-1">{{ ekstrakurikuler.length }}</div>
            <div class="text-red-600 text-sm md:text-base font-medium">Ekstrakurikuler</div>
          </div>
        </div>
      </div>
    </div>
  ...
  <div class="group fixed top-1/2 right-2 transform -translate-y-1/2 z-50" data-aos="fade-left">
    <div class="transition-all duration-300 transform translate-x-5 group-hover:translate-x-0 flex flex-col gap-4">
      <a href="https://instagram.com/psht.smkndu" target="_blank"
      ...
      <a href="https://wa.me/6282338184217" target="_blank"
        class="flex items-center justify-center w-12 h-12 md:w-18 md:h-18 bg-red-800 text-white hover:text-red-800 rounded-l-lg shadow-lg hover:bg-white transition-colors duration-300"
        data-aos="fade-left" data-aos-delay="200" aria-label="WhatsApp">
        <i class="fab fa-whatsapp"></i>
      </a>
      <a href="https://www.tiktok.com/@candratokez1?_t=ZS-8wn5crUZVDt&_r=1" target="_blank"
        class="flex items-center justify-center w-12 h-12 md:w-18 md:h-18 bg-red-800 text-white hover:text-red-800 rounded-l-lg shadow-lg hover:bg-white transition-colors duration-300"
  ...
  <section class="bg-white px-8 py-8 md:px-10 md:py-8" data-aos="fade-up">
    ...
    <div class="flex justify-center gap-4 mb-10">
      <button @click="activeProfile = 'sejarah'" :class="[
        ...
        activeProfile === 'sejarah' ? 'bg-red-800 text-white border-red' : 'text-red-800 border-red hover:bg-red-800 hover:text-white'
      ]">
        Sejarah
      </button>
      <button @click="activeProfile = 'visi_misi'" :class="[
        'px-6 py-2 border-2 rounded-full font-semibold transition-all duration-300',
        activeProfile === 'visi_misi' ? 'bg-red-800 text-white border-red' : 'text-red-800 border-red hover:bg-red-800 hover:text-white'
      ]">
        Visi & Misi
      </button>
    </div>
      ...
      <div v-show="activeProfile === 'sejarah'"
        ...
        <div class="relative w-full h-52 md:h-96 overflow-hidden rounded-2xl shadow-xl" data-aos="zoom-in-right">
          ...
          <button @click="prevprofilSlide"
            class="absolute top-1/2 left-4 transform -translate-y-1/2 z-20 text-white bg-red-200/30 hover:bg-red-200/60 p-2 rounded-full">
            ‹
          </button>
          <button @click="nextprofilSlide"
            class="absolute top-1/2 right-4 transform -translate-y-1/2 z-20 text-white bg-red-200/30 hover:bg-red-200/60 p-2 rounded-full">
            ›
          </button>
        </div>
    ...
    <transition name="fade">
      <div v-show="activeProfile === 'visi_misi'"
        ...
        <!-- Visi -->
        <div
          class="bg-white border-l-4 border-red-800 shadow-lg rounded-xl p-6 flex flex-col gap-4 hover:shadow-xl transition-shadow duration-300"
          data-aos="fade-right" data-aos-delay="150">
          <div class="flex items-center gap-3">
            <i class="fas fa fa-star text-red-800 text-xl"></i>
            <h2 class="text-2xl md:text-2xl font-bold text-red-800">Visi</h2>
          </div>
        ...
        <div
          ...
          <div class="flex items-center gap-3">
            ...
            <h2 class="text-2xl md:text-2xl font-bold text-red-800">Misi</h2>
          </div>
  ...
  <!-- Artikel Populer -->
  <section class="bg-red-800 border-t-8 border-red-800 px-4 py-12" data-aos="fade-up">
    <div class="container mx-auto">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-for="(artikel, index) in popularArticles" :key="artikel.id"
          ...
          <div
            class="absolute right-5 top-5 bg-white rounded-full py-1 px-3 text-sm text-red-800 flex items-center gap-1">
          ...
          <a :href="`/artikel/${artikel.slug}`"
            class="group self-end mb-2 mt-auto inline-flex items-center gap-2 px-3 py-2 text-sm text-white bg-red-800 hover:bg-white hover:text-red-800 transition-all duration-300 rounded-full">
      ...
      <div class="mt-8 text-center" data-aos="fade-up">
        <a href="/artikel"
          class="inline-block text-white border border-white px-6 py-2 rounded-full hover:bg-white hover:text-red-800 transition duration-300">
  ...
  <!-- Pengumuman -->
  <section class="bg-white border-t-8 border-red-800 px-4 py-12" data-aos="fade-up">
    <div class="container mx-auto">
      ...
      <div class="mt-8 text-center" data-aos="fade-up">
        <a href="/pengumuman"
          class="inline-block text-red-800 border border-red-800 px-6 py-2 rounded-full hover:bg-red-800 hover:text-white transition duration-300">
    ...
    <div class="container mx-auto">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
        <div v-for="(item, index) in ekstrakurikuler" :key="item.id"
          class="bg-red-50 border border-red-100 rounded-xl shadow hover:shadow-lg transition-all duration-300 overflow-hidden flex flex-col">
      ...
      <div class="mt-8 text-center" data-aos="fade-up">
        <a href="/ekstrakurikuler"
          class="inline-block text-red-800 border border-red-800 px-6 py-2 rounded-full hover:bg-red-200 hover:text-black transition duration-300">
...
<!-- Video Section -->
<section class="relative bg-gradient-to-br from-black to-red-800 px-4 py-16 overflow-hidden">
  <!-- Background abstract shapes -->
  <div class="absolute top-0 left-0 w-full h-full overflow-hidden opacity-10">
    <div class="absolute -top-20 -left-20 w-64 h-64 rounded-full bg-red-500"></div>
    <div class="absolute bottom-10 right-10 w-80 h-80 rounded-full bg-red-400"></div>
  ...
  <div class="relative max-w-3xl mx-auto text-left md:text-center">
    <!-- Badge -->
    <div class="text-red-700 inline-block mb-3 px-4 py-1 bg-white rounded-full font-bold text-sm" data-aos="fade-down-left">
      MENGENAL LEBIH JAUH
    </div>
    ...
    <div class="relative mb-8" data-aos="flip-left">
      <!-- Glow effect -->
      <div class="absolute -inset-2 bg-gradient-to-tr from-red-400 via-red-500 to-black rounded-xl opacity-75 blur-sm"></div>
      ...
      <div>
        <div class="relative aspect-w-16 aspect-h-9 rounded-xl overflow-hidden shadow-2xl border-4 border-white/10">
          <iframe v-if="videos.length" class="w-full h-full" :src="videos[currentVideoIndex]"
            title="Video Profil SMA Lorem Ipsum" frameborder="0"
            allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
            allowfullscreen></iframe>
          <div v-else class="w-full h-full flex items-center justify-center bg-red-200 text-red-600">
  ...
  <!-- Decorative elements -->
  <div class="hidden md:block absolute -bottom-10 -left-10 w-40 h-40 bg-red-200 rounded-full opacity-50"></div>
